...
def get_matching_chords(chord_notes, chord_structure_name, scale_degree, applied_degree, bass_note, top_note):
    matches = []
...
def search_for_chord(bass_note, top_note, tonic, scale_name):
    ...
    for chord_structure_name, chord_structure in CHORD_STRUCTURES.items():
        for scale_degree, note in enumerate(scale_notes, start=1):
            diatonic_chord_notes = build_chord(chord_structure, note, scale_notes)
            
            diatonic_chords.extend(
                get_matching_chords(diatonic_chord_notes, chord_structure_name, scale_degree, 0, bass_note, top_note)
            )
            ...
            for applied_scale_degree, applied_note in enumerate(applied_scale_notes, start=1):
                ...
                applied_chord_notes = build_chord(chord_structure, applied_note, applied_scale_notes)
                
                applied_chords.extend(
                    get_matching_chords(applied_chord_notes, chord_structure_name, scale_degree, applied_scale_degree, bass_note, top_note)
                )
